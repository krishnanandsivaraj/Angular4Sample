    ...
    Highcharts.Chart.prototype.getDataRows = function () {
        ...
        each(this.series, function (series) {
            if (series.options.includeInCSVExport !== false) {
                ...
                toolTip = ((series.tooltipOptions.valuePrefix) ? series.tooltipOptions.valuePrefix : series.tooltipOptions.valueSuffix);
        ...
        for (x in rows) {
            if (rows.hasOwnProperty(x)) {
                each(rows[x], function (data, ind) {
                    if (isFloat(data)) {
                        rows[x][ind] = toolTip + data.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
                    } else {
                        rows[x][ind] = toolTip + Math.round(data).toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",");
                    }
                });
                rowArr.push(rows[x]);
            }
        ...
        // Add header row
        dataRows = [[xAxis.isDatetimeAxis ? 'DateTime' : 'Report Date'].concat(names)];
